...
| `ChatControllerMixin` | `controller.py` | 命令分发、page 管理、session 生命周期、toggles（339 行）；通过 `_ControllerHost` Protocol 约束对宿主 App 的依赖 |
...
├── controller.py               # ChatControllerMixin — 命令、会话、toggles（339 行）
